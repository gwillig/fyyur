...
def venues():
  ...
  for v in city_state:
    ...
    print(venues_of_v)
    for v_v in venues_of_v:
      venue_dict={}
      venue_dict["id"]=v_v[1]
      venue_dict["name"] = v_v[0]
      venue_dict["num_upcoming_shows"] = db.session.query(Show).filter(Show.start_time >= current_time).filter_by(venue_id = v_v[1]).count()
